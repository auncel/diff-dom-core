...
import RenderTreeXTreeDiff  from './RenderNodeXTreeDiff'
...
function testFactory(question: IFixtureData) {
  ...
  return function(answer: IFixtureData,  expectFunc: Function) {
    test(answer.name, () => {
      ...
      const { newTree } = xTreeDiff.diff<ShadowRenderNode>();
      expectFunc(newTree)
    });
  }
}
...
describe(fixture.title, () => {
  ...
  const answerMap = fixture.answers.reduce((acc, answer) =>  {
